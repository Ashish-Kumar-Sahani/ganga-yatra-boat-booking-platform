...
export default function ConfirmationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl p-6 transition-all duration-300">
        <button
          onClick={onClose}
        ...
        <div className="flex gap-4">
          <div className="rounded-2xl bg-amber-50 dark:bg-amber-950/20 p-3 text-amber-600 shrink-0 self-start">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-405">
              {message}
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-850 pt-4">
          ...
          <button
            onClick={onConfirm}
            disabled={loading}
            className="rounded-xl bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 text-sm font-bold shadow-md shadow-blue-500/10 transition disabled:opacity-50 cursor-pointer"
          >
